...
if(!file.exists("AllBigObjects.RData") | UpdateEverything) {
...
#      cat(reg, " started\n")
    ...
    # Corrs <- sapply(SpNames, function(spm, nsim) {
    #   cat("   doing", spm, "\n")
    #   SimCorrReg(spm, stats="corr", nsim=nsim)
    # }, nsim=nsim)
...
#      cat(reg, " done\n")
  ...
  save(AllCoefs.lqpth, AllCoefs.lqpth.sp, Correlations, file="AllBigObjects.RData")  
} else {
...
ExtractGof <- function(lst.sp, model="maxnet") {
  c(beta = as.numeric(lst.sp$coefficients["Pred"]), AUC = lst.sp$valid["AUC", model],
